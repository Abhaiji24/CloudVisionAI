...
def send_one_request(image_path):
    ...
    if r.status_code != 200:
        ...
        # Make a GET request from the same thread that executed the POST request
        get_response = requests.get(url + '/receive')
        if get_response.status_code == 200:
            print("GET request success:", get_response.text)
        else:
